...
module Make (C : CacheValue) (I : Idx) :
  S with type index := I.t and type cached_value := C.t
  = struct
  type index = I.t
  ...
  module CachedParserOutput = struct
    ...
    let compare (value1, idx1) (value2, idx2) =
      match C.compare value1 value2 with
      | 0 -> I.compare idx1 idx2
      | c -> c
    ...
    let t_of_sexp sexp =
      ...
      | _ -> failwith "Invalid cached parser output"

    let sexp_of_t (value, idx) =
      Sexp.List [
        C.sexp_of_t value;
        I.sexp_of_t idx;
      ]
  ...
  let memo (tag : tag) (parser : cached_value parser) : cached_value parser = fun (idx, callback) ->
    ...
    fun state -> (
      ...
      | None ->
        let new_callback : cached_value parser_callback = fun parser_output -> fun state -> (
          let State state = state in
          match Hashtbl.find state loc with
          | Some (parser_outputs, callbacks) ->
            if Set.mem parser_outputs parser_output then
              State state
            else
              let new_parser_outputs = Set.add parser_outputs parser_output in
              Hashtbl.set state ~key:loc ~data:(new_parser_outputs, callbacks);
              List.fold_left
                callbacks
                ~init:(State state)
        ...
        Hashtbl.set state ~key:loc ~data:(CachedParserOutputSet.empty, [callback]);
  ...
  let run_raw (parser : 'a parser) (input : string) : (parser_position, cached_value parser_output list) Hashtbl.t =
    let index = I.create_from_string input in
    ...
    final_state
    |> Hashtbl.map ~f:(
      fun (parser_outputs, _callbacks) -> Set.to_list parser_outputs
    )
end